...
function ProductsScreen(props){
  ...
  // A classless hook "useState" updates name(state) and setName is the function that updates the state.
  // e.g. name is equivalent to this.state.name, setName is equivalent to this.setName 
  const [modelVisible, setModelVisible] = useState(false); // default value of false
  const [id, setId] = useState(""); // default value of empty string;
  ...
  useEffect(() => {
      if (successSave){ //if successSave is true.
          setModelVisible(false); // hide the form.
      }
  ...
  // Save the value of successSave into the array to compare the value of the next successSave.
  // If the value of the next successSave is different from the one saved in the array,
  // the effect(dispatch action) would be excuted. Same applies to successDelete.
