...
function productListReducers(state = { products: [] }, action){
    ...
    switch (action.type){
        case PRODUCT_LIST_REQUEST:
            return {loading: true, products:[] }; // To show the loading box???
        case PRODUCT_LIST_SUCCESS:
            return {loading: false, products: action.payload}; //set products to the payload
        case PRODUCT_LIST_FAIL:
            return {loading: false, error: action.payload};
        default:
            return state; //original state(no changes here)
    }
}
